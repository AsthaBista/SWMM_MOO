...
i_values = [11, 19, 27, 35, 43, 51]  # Column indices for Terrain Type (BC, GS, TRE)
...
for index, row in df.iterrows():
    ...
    # Apply conditional logic based on the Type column for roofs
    if row.iloc[1] == "5cm":
        base_values = {
    ...
    for i, j in zip(i_values, j_values):
        ...
        if row.iloc[i] == "BC":
            ...
            row_values[f"S{j}_Aimp"] = row.iloc[i + 6]
        
        elif row.iloc[i] == "GS":
            row_values[f"S{j}"] = row.iloc[i + 2]
            row_values[f"S{j}_Aimp"] = row.iloc[i + 6]
        elif row.iloc[i] == "TRE":
            row_values[f"S{j}"] = row.iloc[i + 5]
            row_values[f"S{j}_Aimp"] = row.iloc[i + 6]
