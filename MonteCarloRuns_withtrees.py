...
for sim_number in range(30,len(df)):  # Loop through indices in the csv file
 ...
 #   Assign LID for S1-S4 (columns 3-6). These are roof subcatchments
    ...
    for year in range(yr_start, yr_stop + 1):
        ...
        rpt = read_rpt_file(r'C:\Users\ABI\My_Files\MonteCarlo\SWMMFiles\model_temp.rpt')
        total_precipitation = rpt.runoff_quantity_continuity['Total Precipitation']['Depth_mm']
        evaporation_loss = rpt.runoff_quantity_continuity['Evaporation Loss']['Depth_mm']
        infiltration_loss = rpt.runoff_quantity_continuity['Infiltration Loss']['Depth_mm']
        ...
        results.loc[year - yr_start + 1, 'PRE'] =   total_precipitation
        results.loc[year - yr_start + 1, 'EVA'] =   evaporation_loss
        results.loc[year - yr_start + 1, 'INF'] =  infiltration_loss
